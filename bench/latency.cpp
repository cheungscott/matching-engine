...
std::vector<Command> make_workload(std::size_t n, unsigned seed) {
    ...
    for (std::size_t i = 0; i < n; ++i) {
        ...
        if (r < 30 && next_id > 1) {
            // Cancels dominate real message flow. A mix without
...
RunResult one_run(const std::vector<Command>& warm, const std::vector<Command>& measured) {
    ...
    // D27 — the number above is NOT this engine's throughput. The timed region
...
int main() {
    ...
    auto report = [&](const char* label, const std::vector<std::vector<std::uint64_t>>& runs,
        ...
        // D27 — n is the POOLED sample count across all runs, while every
        ...
        // D27 — max is the median of the five per-run maxima, which is not a
    ...
    // p99.9 FIRST, deliberately. The mean of a latency distribution
