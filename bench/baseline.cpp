...
inline std::uint64_t tsc_now() noexcept {
    ...
    // D27 — the comment says "same rdtscp pair bench_latency uses" and the fence
...
// A re-run got 74-87x and could not reproduce it.
//
// bench_latency's numbers held up better under the same scrutiny for exactly one
// reason: it does its 5-run median INSIDE the binary. Methodology not built into the
// binary is not methodology, so this now does the same and prints the spread
