...
int main() {
    ...
    std::printf("%8s | %-27s | %-27s | %s\n", "depth",
                "add, ns/op (eng / naive)", "cancel, ns/op (eng / naive)", "  ratios");
    std::printf("---------+-----------------------------+"
                "-----------------------------+------------------\n");
    ...
    for (const std::size_t depth : {1'000u, 2'000u, 4'000u, 8'000u, 16'000u}) {
        ...
        std::printf("%8zu | %11.1f / %13.1f | %11.1f / %13.1f |  add %5.1fx  cxl %6.1fx\n",
                    depth, e.build_ns_per_op, n.build_ns_per_op,
                    e.cancel_ns_per_op, n.cancel_ns_per_op,
                    n.build_ns_per_op / e.build_ns_per_op,
                    n.cancel_ns_per_op / e.cancel_ns_per_op);
    }
