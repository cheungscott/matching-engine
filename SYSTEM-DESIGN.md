...
Running log of design decisions, alternatives, and reasoning, recorded as each
decision was made, including the ones later reversed.
...
### D5 - ObjectPool must poison manually (decided before implementation)
- **Finding:** ASan does **not** catch use-after-release in a slab pool by
  ...
  to it. The identical bug fires on `new`/`delete` and is silent on a
...
- **Two constraints:**
  ...
  2. Keep the **free-list link outside the poisoned region**, or walking the
...
> [!NOTE]
> **TRIGGER FIRED AND CLOSED - 2026-09-01**
> Phases 1-4 came in green on **Mon 1 Sep**, a day inside the window, so the trigger
> fired. **Closed the same day:** 8-9 stay out until v0.1 has shipped. The
> decision was not recorded here at the time, which left a reader of this file
...
- **Revisit trigger (Phase 10, measurable):** tight packing without alignment
  ...
  measurement. Good candidate for a measured before/after.

### D8 - Pool safety checks are unconditional; acquire() clears the slot (Phase 1)
Fixes three defects in the pool's first version.

**The defect.** The first version implemented both of `release()`'s safety checks as `assert`. The `Bench`
...
- **Double release:** the second call succeeded, `in_use_` underflowed to `SIZE_MAX`, and the
  next two `acquire()` calls returned **the same slot** - verbatim the corruption the check
...
under Catch2, which has no death tests. That is exactly why the first version's two headline properties had
...
**acquire() now value-initialises the slot.** The first version left the previous occupant's bytes, which was
...
`asan.hpp` rule 2 offers a third option that D5 omitted - "keep the link
...
still lose on the merits; it was never weighed.
...
- **Linear scan to advance a cursor.** The occupancy bitmap + `std::countr_zero` (Blueprint §3.2) is
  the graduation and belongs in Phase 10, after a profiler asks for it. The simple one ships first.
...
removed from it is a bug it can still catch. It stays after Phase 3: it is also the performance
baseline for a before/after write-up, which converts the tick-array and pool choices from asserted to
...
"Phase 4's index retrofit hurting" as one of the risks the plan anticipated. The actual
...
The subtle one is **invariant 2** (index coherence). Walking every level and checking
...
Checking the code against its stated design intent found 25 issues;
these are the ones that changed code.
...
profiler asks for it."* Profiling measured **1,064 scan iterations inside a single operation** on
...
- **`std::flat_map`** - Reference instability on insert
...
Two defects in `bench/latency.cpp`, both found by inspection rather than by use, and both of the kind
...
why the line said so. The file whose stated purpose is *"this program is
...
footnote. The mean of a latency distribution is a number nobody experiences.
...
Blueprint §4.5 calls conservation the single best property test and states it as
...
The uncomfortable part is that a compiler flag found in one second what a careful read-through did not.
...
> quoting a point estimate from a sample with that spread was not. A re-run measured
...
### D25 - Pre-ship check: nine reachable defects, four of them same-day regressions
Three independent audits covered the code,
the tests and the documentation before the v0.1 tag. Every finding below was **demonstrated** with
a running probe under ASan/UBSan, and then **re-verified against the source**
before being fixed. The ship was blocked on them.

The uncomfortable headline: **the previous day's check missed all nine.** It looked for
...
two phases checked each other.** Exactly D19's failure mode, in a form the D19 check did not look
for.
...
2. **Exceptions had not been considered.** `Engine::apply` can throw from three places and has no
...
Every checkable factual claim in `README.md` and `SYSTEM-DESIGN.md` was verified against the
code and a re-run of every benchmark on the same machine.
...
median **inside the binary**, which is why its numbers held up better. Methodology not built into
the binary is not methodology. The rig now does its own repetition.
...
- *"every trade field"* - `seq` is deliberately excluded. D13 documents the exclusion; other write-ups
...
- *"seven invariants across ~1.1M operations"* - the 1M-op case checks **log properties only**. The
  invariants and the differential run over the 100k case. D16 states the split correctly; other
  write-ups compressed it.
...
Measured 2026-09-02: g++ 11.4 `-O2 -DNDEBUG`, WSL2, 200,000 resting orders, 200,000 measured operations,
...
6. **Opportunity cost.** Time on CI is time not spent on the engine; it is justified by the
   compiler-floor claim, not for its own sake.
...
| Have the script compare against canonical and pass/fail | Tempting, and it is a performance gate on an unpinned virtual machine - the exact thing D30 refused to put in CI, for the exact reason it would flap. The script reports; judgement stays manual. |
