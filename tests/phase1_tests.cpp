...
//  not the payload. These tests exist because that was the gap: the first version asserted
...
TEST_CASE("engine_walks_two_price_levels", "[phase3][engine]") {
    // A two-level sweep: 250 resting at 102, 200 at
...
std::vector<scenario::Command> make_stream(unsigned seed, int ops) {
    ...
    for (int i = 0; i < ops; ++i) {
        ...
        if (r < 20 && next_expected_id > 1) {
            // D27. This drew uniformly over EVERY id ever issued, so 92% of
...
TEST_CASE("two market orders differing only in junk price log identically",
    ...
    // identical orders produce byte-identical logs. D27: no test ever fed two
...
TEST_CASE("events_are_emitted_for_every_outcome", "[phase6][events]") {
    ...
    // D27 — InvalidQuantity was produced by NO test anywhere, and MalformedOrder
    ...
    // D27 — the comment above claims a rejected order consumes a sequence number
...
TEST_CASE("the fuzz generator actually exercises the cancel-hit path", "[phase7][audit]") {
    // D27. The generator drew cancel ids uniformly over every id ever issued, so
...
TEST_CASE("the guards that exist to prevent corruption are themselves tested",
          "[phase7][audit]") {
    // D27. Every one of these throws was added in D19/D25 to stop an
...
//
//  Mutation testing neutered every `return false` in OrderBook::is_consistent,
...
//  day's fixes introduced four of the nine, so "fixed" without "pinned" is how
...
TEST_CASE("differential_holds_over_100k_operations_with_invariants", "[.gate][phase7][fuzz]") {
    ...
    for (std::size_t i = 0; i < cmds.size(); ++i) {
        ...
        if ((i + 1) % kConservationEvery == 0) {
            ...
            INFO(c.why);
            REQUIRE(c.ok);
            // D27 — the depth sweep used to run ONCE, after the loop, so a quantity
...
TEST_CASE("the_shrinker_reduces_a_failing_stream", "[.gate][phase7][fuzz]") {
    ...
    // D27. This asserted `< cmds.size() / 10`, i.e. fewer than 400, and the
